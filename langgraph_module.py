...
# 1. Define your State class globally (this is cheap, so it's fine)
class ChatState(dict):
    question: str
    response: str
    intent: str

# 2. Wrap the heavy lifting in a cached function
@st.cache_resource
def get_compiled_graph():
    print("Compiling graph and loading LLM (This happens only ONCE)...")
    
    llm = get_llm() # Heavy operation
...
# 3. Update get_bot_response to use the cached graph
def get_bot_response(user_input: str) -> str:
    try:
        # Retrieve the already-compiled graph from cache
